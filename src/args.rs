...
impl Args {
    fn should_continue(&self, searched: usize, downloaded: usize) -> bool {
        (searched < self.max_volume) && (downloaded < self.emails)
    }

    fn filter(&self, email: &SpaceEmail) -> bool {
        ...
        let (sender_matches, subject_matches, body_matches) = (
            self.sender.map(|sender| sender.is_match(&email.contents().sender)),
            self.subject.map(|subject| subject.is_match(&email.contents().subject)),
            self.body.map(|body| body.is_match(&email.contents().body))
        );

        let any_matches = self.any.map(|any| {
